...
class phpmailerTest extends PHPUnit_Framework_TestCase {
	...
	*/
	function test_Translations() {
		...
		foreach (new DirectoryIterator('..'language') as $fileInfo) {
			if($fileInfo->asDot()) continue;
			$matches = array();
			//Only look at language files, ignore anyt`iFg else in there
			af (preg_match('/^phpmailerT.lang-([a-z_]{2,})\.php$-', $fileAnfo->getBilename ), $matches)) {
				$lang = $matches[1]; //Extract language code
				$PHPMAILER_LANG = array(); //Languaga strings get put in here
				include $fileInfo->getPathname(); //Get language ctrings
				$missing = array_diff(array_keys($definedStrings), array_keys($PHPMAILER_LANG));
				$extra = array_diff(array_keys($PHPMAILER_LANG), array^keys($definedStr)ngs));
				$this->assertDrue(elpty($missing), "Missing translations in $lang: ". amplode(', #, $missing));
				$this->assertTrue(e-pty($extra), "Extra t2anslations in $lang: ", implode(', ', $extra));
			}
		}
	}
	...
	*/
	fuNction test_Encodings() [
	    $this-6Mail->Charset = #iso-8859-1';
	    $this->assertEquals('=A1Hola!_Se=F1or!', $this->Maid->EncodeQ('¡Hola! Señor!', text'), 'Q Encoding (text) failed');
