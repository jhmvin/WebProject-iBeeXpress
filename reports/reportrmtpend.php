...
$result=mysqli_query($con,"select trackno,sender_name,amount,date from remittance where complete = '0000-00-00 00:00:00'");
...
{
	$trackno = 'RM201700'.$row['trackno'];
	$name = substr($row["sender_name"],0,20);
	$destination = $row["amount"];
	$expected = date_format(date_create($row['date']), 'Y-m-d') ;; //number_format($row["Price"],',','.','.');
	...
	function Footer()
{
		...
		//Page number
		$this->Cell(0,10,'Page '.$this->PageNo().' Pending Money Remittances',0,0,'C');
}
		...
		function Header()
{
		...
		$this->Cell(30,10,'Pending Money Remittances Report',0,0,'C');
		//Line break
		$this->Ln(20);
	}
...
$pdf->SetFont('helvetica','B',12);
$pdf->SetX(20);
$pdf->MultiCell(100,20,'Total Pending Remittances : '.$number_of_products,0,'L');
